...
import { BUTTON, GUTTER, HINT, LABEL, LABEL_LIGHT, WORDMARK } from './ledger-style'
import SiteFooter from './site-footer'

/** A section head is a category head: a name, a hairline, and an optional figure. */
  ...
  return (
    <main className="min-h-screen bg-paper text-ink text-[13px] leading-[1.6] sm:text-[14px]">
      <div className="mx-auto max-w-[78ch]">
        {/* No visible headline: the page opens on a finished budget, because the
            document is the argument and anything above it would only describe it.
            The structural heading is therefore read out rather than set. */}
        <h1 className="sr-only">{t('landing.pageTitle')}</h1>
        ...
        <section className={`${GUTTER} pb-[24px]`}>
          <p className={HINT}>{t('landing.heroNote')}</p>
          <Link
            className={`ghost mt-[16px] inline-block ${BUTTON} hover:bg-ink hover:text-paper`}
            href="/generator"
          >
            {t('action.openGenerator')}
          </Link>
        </section>
        ...
        <section className="pt-[24px]">
          ...
          <div className={GUTTER}>
            ...
            <button
              type="button"
              className={`ghost ${BUTTON} cursor-not-allowed`}
              disabled
            >
              {t('action.buyLicense')}
            </button>
